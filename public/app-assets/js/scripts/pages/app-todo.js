...
$(function() {
  ...
  $(".add-todo-item").on('click', function(e){
    ...
    selected.each(function(){
      todoChip += '<div className="chip mb-0">' +
        '<div className="chip-body">' +
          '<span className="chip-text" data-value="'+$(this).data('value')+'"><span className="bullet bullet-'+$(this).data('color')+' bullet-xs"></span> '+$(this).data('value')+'</span>' +
        '</div>' +
      '</div>';
    });
    // HTML Output
    if(todoTitle != ""){
      $(".todo-task-list-wrapper").append('<li className="todo-item" style="animation-delay: 0s;"  data-toggle="modal" data-target="#editTaskModal">' +
        '<div className="todo-title-wrapper d-flex justify-content-between mb-50">' +
          '<div className="todo-title-area d-flex align-items-center">' +
            '<div className="title-wrapper d-flex">'+
              '<div className="vs-checkbox-con">' +
                  '<input type="checkbox" >' +
                  '<span className="vs-checkbox vs-checkbox-sm">' +
                    '<span className="vs-checkbox--check">' +
                      '<i className="vs-icon feather icon-check"></i>' +
                    '</span>' +
                  '</span>' +
              '</div>' +
              '<h6 className="todo-title mt-50 mx-50">'+ todoTitle +'</h6>' +
            '</div>' +
            '<div className="chip-wrapper">' + todoChip + '</div>' +
          '</div>' +
          '<div className="float-right todo-item-action d-flex">' +
            '<a className="todo-item-info'+ todoInfo +'"><i className="feather icon-info"></i></a>' +
            '<a className="todo-item-favorite'+ todoFav +'"><i className="feather icon-star"></i></a>' +
            '<a className="todo-item-delete"><i className="feather icon-trash"></i></a>' +
          '</div>' +
        '</div>' +
        '<p className="mb-0 todo-desc truncate">'+ todoDesc +'</p>' +
      '</li>');
    }
  ...
  $('.update-todo-item').on('click', function(){
    ...
    var $edit_todoChip = "";
        ...
        $edit_selected.each(function(){
          $edit_todoChip += '<div className="chip mb-0">' +
            '<div className="chip-body">' +
              '<span className="chip-text" data-value="'+$(this).data('value')+'"><span className="bullet bullet-'+$(this).data('color')+' bullet-xs"></span> '+$(this).data('value')+'</span>' +
            '</div>' +
          '</div>';
